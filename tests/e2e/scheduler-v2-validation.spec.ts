...
import { grabBottomHandle } from './fixtures/cardPointer';
import { navigateToFutureWeekday } from './fixtures/board';
...
// 날짜 이동은 board.ts 의 navigateToFutureWeekday — "30일" 고정은 일요일(휴무)에 걸리면 전부 invalid 가 된다.
test.describe('SchedulerV2 - 검증 규칙', () => {
  ...
  test('23. drag/resize 중 grid-cell hover overlay (+ 추가)는 표시되지 않는다', async ({ authedPage: page }) => {
    ...
    await page.mouse.move(gx, gy);
    await page.mouse.down();
    await page.mouse.move(gx, gy + 80, { steps: 5 });
    await page.waitForTimeout(300);

    // resize 가 실제로 시작됐는지 먼저 확인 — 시작 자체가 안 되면 overlay 0 은 무의미하다
  ...
  test('24. drag/resize 중 카드의 ⋮ quick action 버튼은 노출되지 않는다 (interaction lock)', async ({ authedPage: page }) => {
    ...
    await page.mouse.move(gx, gy);
    await page.mouse.down();
    await page.mouse.move(gx, gy + 50, { steps: 5 });
    await page.waitForTimeout(300);

...
test.describe('SchedulerV2 - 방문 모드 (TREATMENT)', () => {
  ...
  test('27. TREATMENT 모드의 DateStrip은 오늘 이후 날짜를 표시하지 않는다', async ({ authedPage: page }) => {
    await navigateTreatment(page);
    // CLAUDE.md 정책: 방문 모드 strip = 선택일 기준 -30일, 오늘이 끝
    ...
    // 방문 모드는 ">"(shift-next) 버튼이 today 도달 시 숨겨짐 (v-show)
  ...
  test('30. TREATMENT 모드에서 카드 ⋮ 메뉴는 방문 모드 항목 6개 (대기/완료/미이행/취소/초기화/삭제)', async ({ authedPage: page }) => {
    await navigateTreatment(page);
    // 오늘 또는 과거 날짜의 카드가 1개 이상 있어야 시나리오 동작
    const cardCount = await page.locator('.appointment-card').count();
    test.skip(cardCount === 0, '방문 모드에서 카드가 없음 (LIVE 환경: 오늘 날짜 데이터 없음 가능)');
    ...
    // 방문 모드 메뉴 항목 검증 — 대기는 hover 퀵액션 [대기]와 같은 동작(상태 05)
    await expect(popover.locator('.popover-menu__item', { hasText: '대기' })).toBeVisible();
    ...
    await expect(popover.locator('.popover-menu__item', { hasText: '방문 삭제' })).toBeVisible();
  });
});

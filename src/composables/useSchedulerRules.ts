...
export type RulePriority = 'HOSPITAL_FIRST' | 'DOCTOR_FIRST'; // 룰 우선 순위 HOSPITAL_FIRST: 사업장우선, DOCTOR_FIRST: 담당자우선
...
/** 아무도 그 요일을 정하지 않았을 때의 기본 운영시간 — 사업장 표준 09:00~18:00.
...
export function useSchedulerRules({
     ...
     *
     * <b>공휴일에도 담당자 설정이 우선이다.</b> 담당자가 그 요일을 정해 뒀으면(운영든 휴무가든)
    ...
    function pickDailySchedule(
        ...
        /* -1) 담당자 특정일자 운영 시각이 있으면 **모든 것보다 먼저** 본다 — 담당자 축 안에서
        ...
        if (dateDaily) {
            ...
            if (typeof dd !== 'undefined') {
                /* 담당자가 그 요일을 정해 뒀으면 그것이 답이다 — 운영면 담당자 시각(휴게만 그 날짜 기관 값),
                 * 휴무(null)이면 휴무. 사업장 임시운영 지정은 사업장 축의 값이라 담당자의 명시 휴무를
        ...
        if (isPublicHoliday) {
            ...
            const dd = dr?.weekly?.[weekday as Weekday];
            if (typeof dd !== 'undefined') {
                /* 운영 시작·종료는 담당자 값이지만 **휴게는 그 날짜의 기관 값**을 쓴다.
             ...
             * 종전에는 daily=undefined 를 돌려 그날을 통째로 열었는데(종일운영), 설정·보기 화면과
    ...
    /**
     * 그 날짜에 이 담당자가 **운영으로 정했는가** — 사업장 휴무를 덮는 근거(R11, 사용자 확정).
     * "사업장이 휴무인데 담당자가 휴무가 아니면 담당자를 따른다."
     *
     * 담당자 축 안에서는 구체적인 것이 이긴다 — 자기 특정일자 휴무 > 자기 특정일자 운영 >
     * 자기 매주 휴무 > 자기 운영 요일. 자기 휴무가 하나라도 걸리면 덮지 않는다(그 담당자는 쉬는 날이다).
     ...
     * 설정 화면 `inheritedInstitutionOff` 와 같은 규칙이어야 한다. 갈리면 설정에서 운영으로 보이는 날에
     ...
     *   기관 값은 팀 배치 시점에 이미 복사돼 있다(§3-2). 런타임으로 또 내려받으면 "공휴일에도 운영"로
     ...
     * 규칙이 대신 들어오면, 기관이 운영하는 공휴일에 정한 것 없는 담당자만 쉬게 된다. */
    ...
    function pickReason(date: Date | string | number, groupIdOrNull: string | null | undefined) {
        ...
        // 공휴일 운영일도 같은 성격 — 요일 휴무(closedWeekdays)·요일 dayOffYn 을 무시해야 한다.
        ...
        /* ★임시운영 지정일·공휴일 운영일이 덮는 것은 **사업장** 휴무뿐이다 — 담당자가 명시적으로
         * 휴무로 정해 뒀으면 사업장이 그날 운영한다고 출근시킬 수 없다(§3-1-1 담당자 우선).
         * 종전에는 임시운영 지정일만 담당자 휴무까지 덮었는데(R11 이전 잔재), 휴무일 탭 뷰어는
         * 담당자 매주 휴무를 그대로 휴무로 보여 같은 날짜에 보드와 답이 갈렸다. */

        /* ★공휴일 운영일인데 사업장 공휴일 운영시간이 미설정이어도 **그날은 종일운영다**(휴무 아님).
         * 운영하기로 한 의도를 시간 미입력이 뒤집지 않는다 — 임시운영 지정일(workDates)과 같은 취급이고
        ...
        /* 그 날짜에 이 담당자가 저장한 특정일자 운영 시각. 있으면 요일·기관 시각보다 먼저다. */
        ...
        for (const src of order) {
            ...
            if (opt?.closedDay) {
                ...
                /* 사업장 휴무는 그 담당자가 운영으로 정해 뒀으면 덮인다(R11). 담당자 소스에서도 같은 판정을
                 * 쓴다 — isDoctorWorkOverride 는 자기 휴무(특정일자·매주)을 먼저 보므로 자기 휴무를 자기가
                 * 무시하는 일은 없고, 자기 **특정일자 운영이 자기 매주 휴무를 덮는** 경우(일자 > 요일)만 통과한다. */
                 ...
                 *  - 사업장 소스: 임시운영 지정일·공휴일 운영일이면 그 요일 휴무는 무시한다(기관 축 안에서 일자 > 요일).
                 *  - 담당자 소스: **자기 요일휴무는 아무것도 건너뛰지 않는다.** 사업장의 임시운영 지정은
                 *    담당자 매주 휴무를 덮지 못하고(§3-1-1), 공휴일 운영('Y')도 "공휴일이라는 이유로는 쉬지
                 *    않는다"는 뜻일 뿐이라 요일 휴무를 덮지 않는다(§4-2 2단계) — 매주 금요일 쉬는 담당자가
                 *    금요일 공휴일에 나온다는 결론은 성립하지 않는다. 자기 특정일자 운영만 doctorWorks 로
                 *    요일을 이긴다(일자 > 요일).
                 *    단 **빌려 온 요일휴무**(ownCw 없음 = 그 요일을 정하지 않아 사업장 것을 쓰는 경우)에는
                 *    사업장 규약을 그대로 적용한다 — 기관이 운영하는 공휴일에 미설정 담당자만 쉴 수는 없다. */
            ...
            //   (담당자, 기관) → 내부 priority 재적용으로 이중 swap → 담당자 운영시간이 설정돼 있어도
            ...
            if (daily === null) {
                // 담당자가 선언한 휴무는 임시운영 지정일·공휴일 운영일이 덮지 않는다(위 주석 참조).
            ...
            if (typeof daily === 'undefined') {
                // 공휴일 운영일 + 담당자 미설정 + 기관 공휴일 시간 미설정 → 종일운영(위 주석 참조).
            ...
            // open 정보가 없으면
            if (!hasOpenRange) {
                // holidayWork·공휴일 운영일이면 "하루종일 오픈" 처리(= 밖/점심/저녁 표시 안 뜨게).
            ...
            // CLOSED break = OFF 된 세션이 만든 비운영 구간(open 안이지만 운영 안 함) → 운영종료(outsideHours).
    ...
    function isClosedDayForHeader(
        ...
        // holidayWork면 요일휴무/dayOffYn은 무시 (네 로직 그대로 유지). 공휴일 운영일도 동일.
        const isHolidayOpen = isHolidayOpenDate(date);
        /* 운영시간 소스를 고르는 기준은 "그 날이 공휴일인가"다 — 기관이 그 공휴일에 문을 여는지
         * (isHolidayOpen)와 별개다. 공휴일 축은 담당자 자기 값이라 기관이 쉬어도 그날 여는 담당자가 있다. */
        const isPublicHoliday = isPublicHolidayDate(date);
        const isWorkOverride = isHolidayWorkDate(date) || isHolidayOpen;
        // pickReason 과 같은 규약 — 임시운영 지정일·공휴일 운영일은 담당자가 선언한 휴무까지 덮지는 않는다.

        /* ★공휴일 운영일은 기관 공휴일 운영시간이 미설정이어도 휴무 배지를 띄우지 않는다 — 종일운영다.
        ...
        for (const src of order) {
            ...
            if (unref(blockOptions)?.closedDay) {
                ...
                const cwHit = src === 'hospital'
                    ? inheritsHospitalClosedWeekday(hr, dr, weekday, ymd)
                    : hasInSetOrArray((ownCw ?? fallback?.closedWeekdays ?? null) as any, weekday);
                // pickReason 과 같은 규약 — 담당자가 운영으로 정한 날은 사업장 휴무 배지를 띄우지 않는다(R11).
                const doctorWorks = isDoctorWorkOverride(dr, ymd, weekday);

                if (!doctorWorks && cdHit) return true;
                /* pickReason 과 같은 조건 — 담당자가 정한 매주 휴무는 사업장 임시운영도, 공휴일 운영('Y')도
            ...
            //   인자 swap 으로 담당자 휴무 판정이 기관 daily 로 어긋남.
            const dailyPick = pickDailySchedule(hr, dr, weekday, isPublicHoliday, dateDaily, doctorDateDaily);
            const daily = dailyPick.daily as (DailySchedule | null);
            ...
            if (typeof daily === 'undefined') {
                // 공휴일 운영일인데 기관 공휴일 시간이 미설정이어도 휴무가 아니다 — 종일운영(pickReason 과 같은 규칙).
                continue;
            }
    ...
    function explainBlockedReason(date: Date | string | number, groupIdOrNull: string | null | undefined) {
        ...
        const who = r.source === 'hospital' ? '사업장 공통' : doctorName ? `담당자(${doctorName})` : '담당자';
    ...
    function isHospitalClosedDayForHeader(date: Date | string | number) {
        ...
        // holidayWork면 근무요일로 지정. 공휴일 운영일도 마찬가지로 휴무가 아니다
     ...
     * 담당자 칸에 그대로 쓰면 R11(담당자가 운영으로 정했으면 사업장 휴무를 덮는다)이 셀(pickReason)에는

...
/**
 * 담당자 특정일자 진료 — 사업장 휴무일에 담당자가 그 날짜를 진료로 정한 경우 (2026-08-27).
 *
 * 우선순위(계획서 §3-1-1, R11 사용자 확정):
 *   자기 특정일자 휴무 > 자기 특정일자 진료 > 자기 매주 휴무 > 자기 진료 요일 > (미설정) 사업장 상속
 *
 * 제보: 사업장 8/13 휴무 + 담당자 8/13 진료인데 스케줄러가 '휴무'으로 표기.
 * 원인 둘 —
 *   ① 헤더 뱃지가 쓰는 사업장 휴무 판정(isHospitalClosedDayForHeader)이 담당자를 안 봐 셀은 열리는데
 *      뱃지만 '휴무'이었다 → isHospitalClosedDayForDoctor 신설.
 *   ② 특정일자 진료의 **시각**이 어디서도 소비되지 않아(workDates 플래그만) 요일 시각·기관 폴백으로 열렸고,
 *      담당자 소스에서는 자기 특정일자 진료가 자기 매주 휴무를 덮지 못했다 → doctorRules.dailyByDate 신설.
 *
 ...
 *   kim  = 목 09~17 진료 + 8/13 특정일자 진료 10~14
 *   lee  = 목 매주 휴무 + 8/13 특정일자 진료 10~14   (일자 > 요일)
 *   park = 목 미설정 + 8/13 특정일자 진료 10~14      (기관 폴백 대신 그 시각)
...
function setup(hospitalOverrides: Record<string, unknown> = {}, doctorRules: Record<string, unknown> = makeDoctorRules()) {
  return useSchedulerRules({
    ...
    cellDuration: 30,
    doctorsRef: [
      { id: 'kim', text: '김의사' }, { id: 'lee', text: '이의사' },
      { id: 'park', text: '박의사' }, { id: 'choi', text: '최의사' },
    ],
    options: { priority: 'DOCTOR_FIRST', mergePolicy: 'FALLBACK' },
  })
}

describe('useSchedulerRules — 담당자 특정일자 진료가 사업장 휴무를 덮는다', () => {
  it('★제보 재현: 사업장 휴무일에 담당자가 특정일자 진료면 뱃지·셀 모두 진료다', () => {
  ...
  it('★일자 > 요일: 매주 휴무 요일이어도 특정일자 진료면 그날은 진료다', () => {
    const { getBlockedReason, isClosedDayForHeader, isHospitalClosedDayForDoctor } = setup()
  ...
  it('자기 특정일자 휴무가 가장 먼저다 — 시각이 함께 실려 와도 휴무', () => {
    // 저장 규약상 한 날짜에 휴무와 진료가 공존하지 않지만, 순서 자체를 고정한다.
  ...
  it('휴게는 그 날짜의 사업장 값을 쓴다 — 기관 지정일자 휴게가 있으면 그것', () => {
    // 사업장이 8/13 을 임시진료(09~18, 휴게 12:00~12:30)로 저장한 경우 — 휴무 아님.
    const { getBlockedReason } = setup({
...
/**
 * 사업장 임시진료 지정은 담당자 매주 휴무를 덮지 못한다 — §3-1-1 `자기 매주 휴무 > 사업장 상속` (2026-08-28).
 ...
 */
describe('useSchedulerRules — 사업장 임시진료 지정 vs 담당자 매주 휴무', () => {
  // 사업장: 목요일 매주 휴무인데 8/13 만 임시진료(09~18, BE applyDefaultTime 이 채운 시각).
  const hospital = {
    closedDates: new Set<string>(),
  ...
  it('★매주 휴무 담당자는 그날도 휴무가다 — 셀·뱃지 모두 (휴무일 탭 뷰어와 같은 답)', () => {
    ...
    expect(isHospitalClosedDayForDoctor(D, 'lee'), '사업장은 그날 진료라 기관 휴무 뱃지는 아니다').toBe(false)
  })

  it('미설정 담당자는 사업장을 따라 그날 진료다 — 기관 요일 휴무는 기관 임시진료가 덮는다 (회귀 가드)', () => {

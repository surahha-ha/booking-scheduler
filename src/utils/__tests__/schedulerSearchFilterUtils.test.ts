...
describe('toDisplayStatus (화면별 상태 표시 코드 — 예약 화면은 00·03 만 구분)', () => {
    ...
    it('예약 화면: 완료(01)·미이행(02)·대기(05)는 예약(00)으로 표기', () => {
...
describe('normalizeName (이름 키 정규화)', () => {
    it('특수문자 제거 + trim', () => {
        expect(normalizeName(' 김담당(대표) ')).toBe('김담당대표');
    });
    it('다중 공백 압축(내부 단일 공백은 유지)', () => {
        expect(normalizeName('김  담당자')).toBe('김 담당자');
    });
...
describe('normalizeTeamName (팀명 키 정규화 — 숫자 보존)', () => {
    it('숫자 보존(담당자용 normalizeName 과 달리 충돌 안 함)', () => {
    ...
    it('앞뒤 공백/다중 공백만 정리', () => {
        expect(normalizeTeamName('  1  팀 ')).toBe('1 팀');
    });
});

describe('resolveVisibleDoctors (팀 표시 필터 — 이름 통일 key)', () => {
    const doctors = [
        {id: '김담당', text: '김담당'},
        {id: '최담당', text: '최담당'},
        {id: '이담당', text: '이담당'},
    ...
    const teams = [
        {id: 1, name: '관리팀', doctors: [{staffName: '김담당'}, {staffName: '최담당'}]},
        {id: 2, name: '점검팀', doctors: [{staffName: '이담당'}]},
    ];
    ...
    it('특정 팀(이름) = 담당자 목록 ∩ 그 팀멤버 이름', () => {
        expect(resolveVisibleDoctors('관리팀', doctors, teams).map(d => d.id)).toEqual(['김담당', '최담당']);
    });
    it('빈 문자열도 미지정 취급', () => {
    ...
    it('★회귀: 숫자로만 구분되는 팀명도 각 팀 멤버로 정확히 분리 (1담당 팀≠2담당 팀)', () => {
        const numTeams = [
            {id: 1, name: '1담당 팀', doctors: [{staffName: '김담당'}, {staffName: '최담당'}]},
            {id: 2, name: '2담당 팀', doctors: [{staffName: '이담당'}]},
        ];
        // 버그 시 둘 다 1담당 팀(첫 팀) 멤버로 붕괴됐음 → 각 팀 멤버로 분리돼야 함
        expect(resolveVisibleDoctors('1담당 팀', doctors, numTeams).map(d => d.id)).toEqual(['김담당', '최담당']);
        expect(resolveVisibleDoctors('2담당 팀', doctors, numTeams).map(d => d.id)).toEqual(['이담당']);
    });
    it('팀 없음(빈 teams) → 미지정 = 담당자 전체', () => {
        expect(resolveVisibleDoctors(null, doctors, []).map(d => d.id)).toEqual(['김담당', '최담당', '이담당', '박직원']);
    });
    it('존재하지 않는 팀명 → 빈 결과', () => {
    ...
    it('담당자 이름 특수문자 정규화 매칭 (담당자명 정규화 id vs 팀 raw staffName)', () => {
        const docs = [{id: '김담당대표', text: '김담당대표'}]; // 담당자: 특수문자 제거됨
        const t = [{id: 1, name: '관리팀', doctors: [{staffName: '김담당(대표)'}]}]; // 팀: raw
        expect(resolveVisibleDoctors('관리팀', docs, t).map(d => d.id)).toEqual(['김담당대표']);
    });

    it('담당자 메타(text)를 보존한다', () => {
        expect(resolveVisibleDoctors('점검팀', doctors, teams)).toEqual([{id: '이담당', text: '이담당'}]);
    });

    it('특정 팀 = team.doctors(SORT_ORD) 순서대로 반환 — 담당자 배열 순서 무시 [item1 순서반영]', () => {
        // 담당자 배열은 김담당→최담당 순이지만, 팀 SORT_ORD 는 최담당→김담당 → 그 순서로 나와야 함.
        const t = [{id: 1, name: '관리팀', doctors: [{staffName: '최담당'}, {staffName: '김담당'}]}];
        expect(resolveVisibleDoctors('관리팀', doctors, t).map(d => d.id)).toEqual(['최담당', '김담당']);
    });

    it('팀에 doctors 가 없어도(null) 크래시 없이 동작 — BE 팀 재등록 과도기 방어 (line 134/137 가드)', () => {
        // 팀 삭제·재등록 이력으로 doctors=null 인 빈 팀이 응답에 섞일 수 있음.
        const teamsWithNull = [
            {id: 1, name: '관리팀', doctors: [{staffName: '김담당'}]},
            {id: 2, name: '빈팀', doctors: undefined},
        ] as any;
        // 미지정(line 137): 관리팀 멤버(김담당) 제외, null 팀은 멤버 0 취급
        expect(() => resolveVisibleDoctors(null, doctors, teamsWithNull)).not.toThrow();
        expect(resolveVisibleDoctors(null, doctors, teamsWithNull).map(d => d.id)).toEqual(['최담당', '이담당', '박직원']);
        // 특정 팀이 doctors null(line 134): 빈 결과
        expect(resolveVisibleDoctors('빈팀', doctors, teamsWithNull)).toEqual([]);
    });
});

// 기대값 출처: 상수 APPOINTMENT/TREATMENT_STATUS_TYPE 의 주석(예약 00 · 취소 03 · 대기 05 · 완료 01 · 미이행 02).
...
describe('toStatusLabel (표시 상태 코드 → 그 장부의 칩 라벨)', () => {
    ...
    it('방문장부: 05 → 대기, 01 → 완료, 02 → 미이행, 03 → 취소', () => {
    ...
    it('그 장부에 칩이 없는 코드는 null — 방문장부의 00(예약), 예약장부의 01(완료)', () => {

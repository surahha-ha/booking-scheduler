...
/** 기관은 월~토 09~18 전부 진료 — 차단이 나오면 그 출처는 담당자뿐이다. */
...
function boardRules(store: ReturnType<typeof useStaffStore>) {
  return useSchedulerRules({
    ...
    selectedDoctors: new Set(['김의사']),
    cellDuration: 30,
    doctorsRef: [{ id: '김의사', text: '김의사' }],
...
describe('staffOffSettingsInput — 담당자 휴무 원천 → buildHolidayClosure 입력', () => {
  ...
  it('특정일자: 시분 없으면 휴무(offDates) · 있으면 진료(workDates=rescue)', () => {
    const input = staffOffSettingsInput({}, [
...
describe('loadWorkHours — 담당자 휴무일이 doctorRules 에 실린다', () => {
  ...
  it('특정일자 휴무가 closedDates 에 들어간다', async () => {
    ...
    expect([...(store.doctorRules['김의사']!.closedDates as Set<string>)]).toContain(MON)
  })
  ...
  it('매주 휴무 요일이 closedWeekdays 에 들어간다 (weekly=null 과 별개로 가산)', async () => {
    ...
    const rule = store.doctorRules['김의사']!
  ...
  it("공휴일 'N' 이면 공휴일이 그 담당자 closedDates 에 합류한다", async () => {
    ...
    expect([...(store.doctorRules['김의사']!.closedDates as Set<string>)]).toContain(TUE)
  })

  it('특정일자 진료(시분 있음)는 매월 휴무를 구제한다 — 일자 지정이 반복 규칙보다 구체적이다', async () => {
    const store = await loadWith({
      staff: [staff({ monthlyOffRules: [{ dayCd: 1, monthlyNth: 4 }] })],
      overrides: [{ staffId: 101, date: MON, overrideOpenHm: '1000', overrideCloseHm: '1500' }],
    })
    expect([...(store.doctorRules['김의사']!.closedDates as Set<string>)]).not.toContain(MON)
  })

  it('override 는 staffId 로 갈린다 — 남의 휴무일이 옮아붙지 않는다', async () => {
    const store = await loadWith({
      staff: [staff(), staff({ staffId: 102, staffName: '이의사' })],
      overrides: [{ staffId: 102, date: MON, overrideOpenHm: null, overrideCloseHm: null }],
    })
    expect([...(store.doctorRules['이의사']!.closedDates as Set<string>)]).toContain(MON)
    expect([...(store.doctorRules['김의사']!.closedDates as Set<string>)]).not.toContain(MON)
  })
  ...
  it('holidayOpenDates 는 담지 않는다 — 소비처(useSchedulerRules)가 사업장 값만 읽는다', async () => {
    ...
    expect(store.doctorRules['김의사']!.holidayOpenDates).toBeUndefined()
  })
})
...
describe('보드 반영 실측 — store 를 채우면 차단 사유가 담당자으로 잡힌다', () => {
  ...
  it('담당자 특정일자 휴무 → "의사(김의사) 휴무일(2026-08-24)"', async () => {
    const store = await loadWith({
      staff: [staff()],
      overrides: [{ staffId: 101, date: MON, overrideOpenHm: null, overrideCloseHm: null }],
    })
    ...
    expect(getBlockedReason(at, '김의사').reason).toBe('closedDate')
    expect(explainBlockedReason(at, '김의사')).toBe(`의사(김의사) 휴무일(${MON})`)
  })
  ...
  it('담당자 매월 N번째 휴무 → 같은 경로로 휴무일 차단', async () => {
    ...
    expect(explainBlockedReason(`${MON}T10:00:00`, '김의사')).toBe(`의사(김의사) 휴무일(${MON})`)
  })
  ...
  it('담당자 매주 휴무 → "휴무" 가 아니라 "휴무요일(월)" 로 사유가 좁혀진다', async () => {
    ...
    const { explainBlockedReason } = boardRules(store)
    expect(explainBlockedReason(`${MON}T10:00:00`, '김의사')).toBe('의사(김의사) 휴무요일(월)')
  })
  ...
  it('휴무일이 아닌 날은 열려 있다 (차단이 날짜 밖으로 번지지 않는다)', async () => {
    ...
    expect(getBlockedReason(`${TUE}T10:00:00`, '김의사').blocked).toBe(false)
  })

  it('담당자가 미설정인 요일은 사업장 휴무일을 그대로 상속한다', async () => {
    // 수요일(3)은 김의사가 운영시간을 정하지 않았다 → 기관 판정으로 내려간다.
    ...
    const { getBlockedReason } = boardRules(store)
    const r = getBlockedReason(`${WED}T10:00:00`, '김의사')
 ...
 *   자기 특정일자 휴무 > 자기 특정일자 진료 > 자기 매주 휴무 > 자기 진료 요일 > (미설정) 사업장 상속.
 */
describe('R11 — 담당자 설정이 사업장 휴무를 덮는다', () => {
  ...
  it('운영시간을 정해 둔 요일이면 사업장 특정일자 휴무를 덮는다', async () => {
    ...
    expect(boardRules(store).getBlockedReason(`${TUE}T10:00:00`, '김의사').blocked).toBe(false)
  })
  ...
  it('운영시간을 정해 둔 요일이면 사업장 매주 휴무 요일도 덮는다', async () => {
    ...
    expect(boardRules(store).getBlockedReason(`${MON}T10:00:00`, '김의사').blocked).toBe(false)
  })
  ...
  it('특정일자 진료는 운영시간 미설정 요일에도 사업장 휴무를 덮는다', async () => {
    const WED = '2026-08-26'
    const store = await loadWith({
      staff: [staff()],
    ...
    expect(boardRules(store).getBlockedReason(`${WED}T10:00:00`, '김의사').blocked).toBe(false)
  })
  ...
  it('★자기 휴무가 먼저다 — 매주 휴무 요일에는 사업장 휴무를 덮지 않는다', async () => {
    ...
    expect(boardRules(store).getBlockedReason(`${MON}T10:00:00`, '김의사').blocked).toBe(true)
  })

  it('★자기 특정일자 휴무가 자기 진료 요일보다 먼저다', async () => {
    const store = await loadWith({
      staff: [staff()],   // 월요일 09~18 진료
      overrides: [{ staffId: 101, date: MON, overrideOpenHm: null, overrideCloseHm: null }],
    })
    store.hospitalRules.closedDates = new Set([MON])

    const r = boardRules(store).getBlockedReason(`${MON}T10:00:00`, '김의사')
    expect(r.blocked).toBe(true)
  ...
  it('덮더라도 운영시간 밖은 여전히 막힌다 — 휴무 판정만 덮지 시간을 늘리지 않는다', async () => {
    ...
    const r = boardRules(store).getBlockedReason(`${TUE}T20:00:00`, '김의사')
    expect(r.blocked).toBe(true)
...
// ════════════════════════════════════════════════════════════
/**
 * 특정일자 진료의 **시각**이 보드에 실린다 (2026-08-27).
 * 종전에는 override 를 "그날 진료한다"(workDates) 로만 접어 값을 버렸고, 그 날짜는 요일 시각(또는 기관 폴백)으로
 * 열렸다 — 10~15 로 저장했는데 09~18 이 열리는 상태. 또 담당자 소스에서 자기 특정일자 진료가 자기 매주 휴무를
 * 덮지 못해(일자 > 요일 미준수) 매주 휴무 요일의 특정일자 진료가 '휴무'으로 나왔다.
 */
describe('특정일자 진료 시각 → doctorRules.dailyByDate (일자 > 요일 > 사업장)', () => {
  beforeEach(() => {
    setActivePinia(createPinia())
    mocks.getSiteWorkHours.mockReset()
    mocks.getStaffWorkHours.mockReset()
  ...
  it('시분 있는 override 는 dailyByDate 에 시각으로 담긴다 · 시분 없는 override 는 담지 않는다', async () => {
    ...
    const byDate = store.doctorRules['김의사']!.dailyByDate!
  ...
  it('★그 날짜는 저장한 시각(10~15)으로 열린다 — 요일 시각·기관 폴백으로 넓히지 않는다', async () => {
    ...
    expect(rules.getBlockedReason(`${WED}T09:30:00`, '김의사').blocked, '10:00 전').toBe(true)
    expect(rules.getBlockedReason(`${WED}T12:00:00`, '김의사').blocked, '안').toBe(false)
    const late = rules.getBlockedReason(`${WED}T16:00:00`, '김의사')
    expect(late.blocked, '15:00 후').toBe(true)
    expect(late.reason).toBe('outsideHours')
  })

  it('★일자 > 요일: 매주 휴무 요일의 특정일자 진료는 진료다 — 셀·뱃지 모두', async () => {
    ...
    const rules = boardRules(store)
    expect(rules.getBlockedReason(`${MON}T12:00:00`, '김의사').blocked).toBe(false)
    expect(rules.isClosedDayForHeader(MON, '김의사')).toBe(false)
    expect(rules.isHospitalClosedDayForDoctor(MON, '김의사')).toBe(false)
    // 다음 주 월요일은 그대로 매주 휴무가다.
    const nextMon = dayjs(MON).add(7, 'day').format('YYYY-MM-DD')
    expect(rules.getBlockedReason(`${nextMon}T12:00:00`, '김의사').blocked).toBe(true)
  })

  it('★제보 재현: 사업장 휴무 + 담당자 특정일자 진료 → 담당자 칸 뱃지는 휴무가 아니다 (날짜 행은 휴무)', async () => {
    ...
    expect(rules.isHospitalClosedDayForDoctor(WED, '김의사'), '담당자 칸').toBe(false)
    expect(rules.isClosedDayForHeader(WED, '김의사')).toBe(false)
  })

  it('override 가 없는 담당자 칸은 사업장 휴무 뱃지를 그대로 받는다', async () => {
    const store = await loadWith({ staff: [staff(), staff({ staffId: 102, staffName: '박의사' })], overrides: [override] })
    store.hospitalRules.closedDates = new Set([WED])
    store.hospitalRules.designatedOffDates = new Set([WED])

    const rules = boardRules(store)
    expect(rules.isHospitalClosedDayForDoctor(WED, '박의사')).toBe(true)
    expect(rules.getBlockedReason(`${WED}T12:00:00`, '박의사').reason).toBe('closedDate')
  })
})

// ════════════════════════════════════════════════════════════
/**
 * 공휴일 진료 'Y' 인데 그 공휴일이 자기 매주 휴무 요일 — **그날은 휴무가다** (계획서 §4-2 2단계, 2026-08-28 확정).
...
describe("공휴일 'Y' + 매주 휴무 요일의 공휴일 → 요일 휴무가 이긴다", () => {
  ...
  it('기관 공휴일 시간이 있어도 채우지 않는다 — 요일 휴무가 이긴다', async () => {
    ...
    expect(store.doctorRules['김의사']!.dailyByDate![FRI_HOLIDAY]).toBeUndefined()
  })
  ...
  it('★보드도 그날 휴무가다 — 휴무일 탭 뷰어와 같은 답. 다음 주 같은 요일도 그대로 휴무', async () => {
    ...
    const rules = boardRules(store)
    expect(rules.getBlockedReason(`${FRI_HOLIDAY}T11:00:00`, '김의사').blocked).toBe(true)
    expect(rules.isClosedDayForHeader(FRI_HOLIDAY, '김의사')).toBe(true)
    const nextFri = dayjs(FRI_HOLIDAY).add(7, 'day').format('YYYY-MM-DD')
    expect(rules.getBlockedReason(`${nextFri}T11:00:00`, '김의사').blocked).toBe(true)
  })

  it("'N' 이거나 진료 요일이면 채우지 않는다 — 기존 공휴일 경로 그대로", async () => {
    holidayMock.fetchPublicHolidays.mockResolvedValue([FRI_HOLIDAY])
    const n = await loadWith({ staff: [staff({ times: [{ dayCd: 5, staffOpenHm: null, staffCloseHm: null }], holidayOpenYn: 'N' })], overrides: [] })
    expect(n.doctorRules['김의사']!.dailyByDate![FRI_HOLIDAY]).toBeUndefined()

    setActivePinia(createPinia())
    const y = await loadWith({ staff: [staff({ times: [{ dayCd: 5, staffOpenHm: '0900', staffCloseHm: '1300' }], holidayOpenYn: 'Y' })], overrides: [] })
    expect(y.doctorRules['김의사']!.dailyByDate![FRI_HOLIDAY]).toBeUndefined()
  })
  ...
  it('채울 기관 시간이 하나도 없으면 담지 않는다 — 요일 휴무가 남는다 (적용할 시간이 없으면 휴무)', async () => {
    ...
    expect(store.doctorRules['김의사']!.dailyByDate![FRI_HOLIDAY]).toBeUndefined()
    expect(boardRules(store).getBlockedReason(`${FRI_HOLIDAY}T11:00:00`, '김의사').blocked).toBe(true)
  })
})
 ...
 * 갈리면 설정에서 진료로 보이는 날에 보드가 예약을 막거나, 그 반대가 된다.
 */
describe('축 단위 상속 — 보드가 휴무일 탭과 같은 답을 낸다', () => {
  ...
  /* ★수요일로 본다 — staff() 기본값은 월·화에만 운영시간이 있어, 그 두 요일은 R11(진료로 정한 요일이
   *  사업장 휴무를 이긴다)이 먼저 걸려 상속 여부가 드러나지 않는다. 수요일은 양쪽 다 미설정이다. */
  const WED = '2026-08-26'

  it('★요일 축을 스스로 정한 담당자는 사업장 매주 휴무를 따라가지 않는다', async () => {
    // 김의사는 화요일 하나만 휴무로 정했다 → 요일 축이 자기 것이 된다. 박의사는 아무것도 정하지 않았다.
    const store = await loadWithSettings(
      { recurringOffRules: [{ dayCd: 3, repeatTy: 'WEEKLY', monthlyNth: null }] },   // 기관 수요일 휴무
      {
        staff: [
          ...
          staff({ staffId: 102, staffName: '박의사' }),
        ],
        overrides: [],
      },
    )
    ...
    expect(rules.getBlockedReason(`${WED}T10:00:00`, '김의사').blocked, '기관 수요일 휴무를 안 따른다').toBe(false)
    expect(rules.getBlockedReason(`${TUE}T10:00:00`, '김의사').blocked, '자기 화요일 휴무는 그대로').toBe(true)
    expect(rules.getBlockedReason(`${WED}T10:00:00`, '박의사').blocked, '정한 것이 없으면 상속').toBe(true)
  })

  it('★요일 축을 소유한 담당자의 미설정 요일에는 어느 휴무 배지도 붙지 않는다', async () => {
    /* 제보: 화요일만 휴무로 정한 담당자의 일요일 칸에 사업장 '휴무' 배지(회색)가 떴다.
     * 기관 일요일은 매주 휴무가라 **운영시간 행 자체가 없고**(siteResponse 는 월~토만),
    ...
    const store = await loadWithSettings(
      { recurringOffRules: [{ dayCd: 0, repeatTy: 'WEEKLY', monthlyNth: null }] },   // 기관 일요일 휴무
      {
        staff: [
          staff({ times: [{ dayCd: 2, staffOpenHm: null, staffCloseHm: null }] }),
          staff({ staffId: 102, staffName: '박의사' }),   // 아무것도 정하지 않음 → 기관 상속
        ],
        overrides: [],
      },
    )
    ...
    expect(rules.isHospitalClosedDayForDoctor(SUN, '김의사'), '기관 휴무를 안 따르니 회색 배지 없음').toBe(false)
    expect(rules.isClosedDayForHeader(SUN, '김의사'), '담당자 휴무 배지도 없음').toBe(false)
    /* 정한 것이 없으면 기본 09:00~18:00 으로 연다 — 기관 휴무 요일이라 운영시간 행이 없다는 이유로
     * 막으면, 축을 끊어 놓고도 결과가 그대로라 규칙이 무력해진다(사용자 확정 2026-08-28). */
    expect(rules.getBlockedReason(`${SUN}T10:00:00`, '김의사').blocked, '기본 운영시간으로 열린다').toBe(false)
    expect(rules.isClosedDayForHeader(TUE, '김의사'), '자기가 정한 화요일은 그대로 휴무').toBe(true)

    expect(rules.getBlockedReason(`${SUN}T10:00:00`, '박의사').blocked, '상속하는 담당자는 그대로 휴무').toBe(true)
    expect(rules.isHospitalClosedDayForDoctor(SUN, '박의사'), '그 칸에는 회색 배지가 뜬다').toBe(true)
    expect(rules.isHospitalClosedDayForHeader(SUN), '날짜 행은 병원 휴무 그대로').toBe(true)
  })
  ...
  it('★일자 축을 스스로 정한 담당자는 사업장 일자 지정을 따라가지 않는다', async () => {
    ...
    const store = await loadWithSettings(
      ...
      {
        staff: [staff(), staff({ staffId: 102, staffName: '박의사' })],
        // 김의사만 다른 날짜에 자기 지정이 있다 → 일자 축이 자기 것이 된다
        overrides: [{ staffId: 101, date: OTHER, overrideOpenHm: null, overrideCloseHm: null }],
      },
    )
    const rules = boardRules(store)

    expect(rules.getBlockedReason(`${WED}T10:00:00`, '김의사').blocked, '기관 지정을 안 따른다').toBe(false)
    expect(rules.getBlockedReason(`${WED}T10:00:00`, '박의사').blocked, '정한 것이 없으면 상속').toBe(true)
  })

  it('★사업장 일자 임시휴무는 공휴일 진료 담당자도 덮는다 — 뷰어와 같은 답', async () => {
    /* 한때 store 가 공휴일 진료 'Y' 인 담당자의 workDates 에 공휴일 전부를 담았다. 그래서 그 날짜가
     * 통째로 "담당자가 진료로 정한 날"이 되어 **사업장이 콕 집어 지정한 임시휴무까지** 덮었고,
     * 뷰어는 휴무 · 보드는 진료로 갈렸다. 'Y' 는 요일도 일자 지정도 덮지 않는다. */
    holidayMock.fetchPublicHolidays.mockResolvedValue([HOLIDAY])
    const store = await loadWithSettings(
      { offDates: [HOLIDAY] },
      { staff: [staff({ holidayOpenYn: 'Y' })], overrides: [] },
    )
    const r = boardRules(store).getBlockedReason(`${HOLIDAY}T11:00:00`, '김의사')
  ...
  it('★사업장 공휴일 휴무는 담당자에게 상속되지 않는다 — 공휴일은 늘 자기 값이다', async () => {
     ...
     * 런타임으로 또 내려받으면 "공휴일에도 진료('Y')"로 정해 둔 담당자가 기관 휴무를 상속해 휴무가 된다. */
    holidayMock.fetchPublicHolidays.mockResolvedValue([HOLIDAY])
    const store = await loadWithSettings(
    ...
    expect(rules.getBlockedReason(`${HOLIDAY}T11:00:00`, '김의사').blocked, '셀').toBe(false)
    expect(rules.isHospitalClosedDayForDoctor(HOLIDAY, '김의사'), '뱃지').toBe(false)
  ...
  it("★공휴일에는 사업장 요일 휴무를 상속하지 않는다 — 기관 축도 공휴일이면 요일을 보지 않는다", async () => {
    ...
    expect(rules.getBlockedReason(`${HOLIDAY}T11:00:00`, '김의사').blocked).toBe(false)
    // 같은 요일이지만 공휴일이 아닌 날은 종전대로 상속한다
    const plainFri = dayjs(HOLIDAY).add(7, 'day').format('YYYY-MM-DD')
    expect(rules.getBlockedReason(`${plainFri}T11:00:00`, '김의사').blocked, '평소 금요일은 상속').toBe(true)
  })
  ...
  it("담당자가 공휴일 휴무 'N' 이면 그 날은 자기 값으로 휴무가다", async () => {
    ...
    expect(boardRules(store).getBlockedReason(`${HOLIDAY}T11:00:00`, '김의사').blocked).toBe(true)
  })
})

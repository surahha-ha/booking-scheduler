 ...
 * 전원이 팀에 배정돼 그 그룹이 비면 골라도 의사 0명·컬럼 0개인 빈 화면이 되므로 선택지에서 감춘다.
...
// 진료 팀은 siteApi 소관이다(staffApi 아님).
vi.mock('@/api/siteApi', () => ({
...
/** 진료 팀 응답 — teams[].doctors 가 그 팀의 멤버 */
...
describe("팀 셀렉트의 '미지정' 선택지", () => {
    ...
    it('전원이 팀에 배정돼 팀 미소속이 0명이면 노출하지 않는다', async () => {
        mocks.getDoctors.mockResolvedValue(doctorsResp(['김의사', '이의사']));
        mocks.getTeams.mockResolvedValue(teamsResp([{name: '보철팀', members: ['김의사', '이의사']}]));

        const wrapper = await mountFilter();

        expect(teamOptions(wrapper)).toEqual(['보철팀']);
    });

    it('팀 미소속 담당자가 남아 있으면 노출한다', async () => {
        mocks.getDoctors.mockResolvedValue(doctorsResp(['김의사', '이의사']));
        mocks.getTeams.mockResolvedValue(teamsResp([{name: '보철팀', members: ['김의사']}]));

        const wrapper = await mountFilter();

        expect(teamOptions(wrapper)).toEqual(['보철팀', '미지정']);
    });

    it('원장에 선등록된 "미지정" 은 팀 멤버가 될 수 없으므로 선택지가 유지된다', async () => {
        mocks.getDoctors.mockResolvedValue(doctorsResp(['김의사', '미지정']));
        mocks.getTeams.mockResolvedValue(teamsResp([{name: '보철팀', members: ['김의사']}]));
    ...
    it('팀이 하나도 없으면 전원이 미소속이므로 선택지가 유지된다', async () => {
        mocks.getDoctors.mockResolvedValue(doctorsResp(['김의사']));

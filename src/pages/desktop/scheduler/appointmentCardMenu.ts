 ...
 *
 * 분기축은 화면(dataType: 예약/방문) + 예약 상태 코드다.
...
/** 초기화는 예약·방문 공통. 취소·미이행·완료·대기 건을 되살리는 유일한 경로다. */
...
export function buildCardMenu(dataType: DataType, status: string): CardMenuButton[] {
  ...
  return [
    // 대기: 카드 hover 퀵액션 [대기]와 같은 동작(상태 05). 이미 05 면 할 일이 없다.
    { value: 'WAITING', displayLabel: '대기', disabled: status === '05' },
    { value: 'COMPLETE', displayLabel: '완료' },
    { value: 'NOSHOW', displayLabel: '미이행' },
    { value: 'CANCEL', displayLabel: '취소' },
    restoreButton(status),
    { value: 'DELETE', displayLabel: '방문 삭제' },
  ]
}

 ...
 *  ③ 연 시점의 (일자·시작시각·의사)와 같으면 묻지 않는다(진입 확인과 두 번 겹치지 않게).
...
const mocks = vi.hoisted(() => ({
  ...
  /** 진료항목 마스터 — 테스트마다 갈아끼운다(빈 그룹 저장 차단 검증용). */
...
describe('ReservationPopup — 고를 항목이 없는 그룹은 저장하지 않는다', () => {
  ...
  it('그룹에 항목이 하나도 없으면 진료항목을 null 로 보낸다', async () => {
...
describe('ReservationPopup — 설정 팝업을 닫으면 고른 진료항목을 마스터에 맞춘다', () => {
...
// 진료항목은 입력칸이 없어 다른 필수값처럼 테두리로 알릴 수 없다 — 이 경로만 alert 다.
...
describe('ReservationPopup — 그룹은 골랐는데 항목을 안 고르면 alert 로 막는다', () => {
  ...
  it('항목이 있는 그룹을 골라 두고 항목 없이 수정하면 alert 1회·저장 없음·버튼은 미완성 표시만', async () => {
    ...
    expect(mocks.alert).toHaveBeenCalledWith(
      '선택한 그룹의 진료항목을 선택해주세요.',
      expect.objectContaining({ title: '진료항목 선택' }),
    )
...
describe('ReservationPopup — 저장 직전 운영시간 재확인', () => {
  it('③ 연 시점과 일자·시작시각·의사가 같으면 확인을 묻지 않는다', async () => {
...
describe('ReservationPopup — 상태 뱃지', () => {
  ...
  it('예약 화면 접수대기(05): 뱃지를 감춘다 — 00·03 만 구분하는 규칙이다', async () => {
    const wrapper = await openPopup(blockedAsLunch(), { status: '05' })

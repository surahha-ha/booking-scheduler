 ...
 *
 * UiDoctorFilter — 담당자 체크박스 필터 (예약장부 검색필터 / 운영일정 보기 공용).
 *
...
const DOCTORS = [
  { value: 'A담당자', label: 'A담당자' },
  { value: 'B담당자', label: 'B담당자' },
  { value: 'C담당자', label: 'C담당자' },
]
...
describe('UiDoctorFilter — 선택 규약 (빈 배열 = 전체)', () => {
  it('빈 배열이면 "전체"가 켜지고 개별 담당자는 꺼진다', () => {
    const wrapper = mountFilter({ modelValue: [] })

    expect(isChecked(wrapper, '전체')).toBe(true)
    expect(isChecked(wrapper, 'A담당자')).toBe(false)
    expect(isChecked(wrapper, 'B담당자')).toBe(false)
  })

  it('전체 상태에서 한 명을 고르면 그 한 명만 나간다', async () => {
    const wrapper = mountFilter({ modelValue: [] })

    await clickItem(wrapper, 'B담당자')
    expect(lastEmitted(wrapper)).toEqual(['B담당자'])
  })

  it('선택 상태를 modelValue 로 받아 그대로 표시한다 (v-model 계약)', () => {
    const wrapper = mountFilter({ modelValue: ['B담당자'] })

    expect(isChecked(wrapper, '전체')).toBe(false)
    expect(isChecked(wrapper, 'B담당자')).toBe(true)
    expect(isChecked(wrapper, 'A담당자')).toBe(false)
  })
  ...
  it('★전원을 고르면 빈 배열로 접힌다 — "전체"와 같은 뜻이기 때문', async () => {
    const wrapper = mountFilter({ modelValue: ['A담당자', 'B담당자'] })

    await clickItem(wrapper, 'C담당자')
    expect(lastEmitted(wrapper)).toEqual([])
  })
  ...
  it('여러 명을 순서대로 고를 수 있다', async () => {
    const wrapper = mountFilter({ modelValue: ['A담당자'] })

    await clickItem(wrapper, 'B담당자')
    expect(lastEmitted(wrapper)).toEqual(['A담당자', 'B담당자'])
  })

  it('★담당자가 한 명뿐이면 그를 눌러도 전체 그대로다 — 그 한 명이 곧 전원이라 바뀔 것이 없다', async () => {
    // 1인 사업장. [값] 을 내보내면 화면은 계속 '전체'인데 조회 조건만 달라지고,
    // 다시 눌러도 같은 값이 재-emit 되어 되돌릴 수 없게 된다.
    const wrapper = mountFilter({ buttonItems: [DOCTORS[0]], modelValue: [] })

    await clickItem(wrapper, 'A담당자')
...
describe('UiDoctorFilter — 표시할 담당자가 없을 때', () => {
  ...
  it('표시할 담당자가 없을 때 "전체"를 눌러도 아무 일도 없다 — 이미 전체라 재조회를 부르지 않는다', async () => {
    const wrapper = mountFilter({ buttonItems: [], modelValue: [] })
  ...
  it('담당자가 있다가 사라지면 선택이 풀리고 "전체"가 켜진다', async () => {
    const wrapper = mountFilter({ modelValue: ['B담당자'] })
...
describe('UiDoctorFilter — 목록이 바뀔 때 (normalize)', () => {
  it('목록에서 사라진 담당자는 선택에서 빠진다', async () => {
    const wrapper = mountFilter({ modelValue: ['A담당자', 'C담당자'] })

    // C담당자가 없는 팀으로 교체 — 남은 선택만 유지되어야 한다
    await wrapper.setProps({ buttonItems: [DOCTORS[0], DOCTORS[1]] })
    expect(lastEmitted(wrapper)).toEqual(['A담당자'])
  })

  it('선택이 전부 사라지면 전체(빈 배열)로 되돌아간다', async () => {
    const wrapper = mountFilter({ modelValue: ['C담당자'] })
...
describe('UiDoctorFilter — 표시', () => {
  it('"전체"가 목록 맨 앞에 함께 나온다 (FIX_ALL=false)', () => {
    ...
    expect(labels(wrapper)).toEqual(['전체', 'A담당자', 'B담당자', 'C담당자'])
  })

  it('비공개 담당자에는 뱃지가 붙는다', () => {
    const wrapper = mountFilter({
      buttonItems: [{ value: 'A담당자', label: 'A담당자', isPrivate: true }, DOCTORS[1]],
    })
...
describe('UiDoctorFilter — 페이지 이동 (한 쪽 5개)', () => {
  const MANY = Array.from({ length: 7 }, (_, i) => ({ value: `담당자${i}`, label: `담당자${i}` }))

  it('5개를 넘으면 나눠 보여준다 — "전체"도 한 자리를 차지한다', () => {
    const wrapper = mountFilter({ buttonItems: MANY })
    // [전체, 담당자0..담당자3] = 5개
    expect(labels(wrapper)).toEqual(['전체', '담당자0', '담당자1', '담당자2', '담당자3'])
  })
  ...
  it('다음을 누르면 나머지가 나온다', async () => {
    ...
    expect(labels(wrapper)).toEqual(['담당자4', '담당자5', '담당자6'])
  })
  ...
  it('목록이 줄어 쪽수가 모자라면 보던 쪽을 되돌린다', async () => {
    ...
    expect(labels(wrapper)).toEqual(['전체', '담당자0', '담당자1'])
  })
})
